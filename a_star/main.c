...
int main(){
	...
	push(list,start_node,0,h(0));
	if(a_star(g,list,goal_node)){
		while(goal_node != -1){
			printf("%d, ",goal_node);
...
void push(p_queue *list,int v,int c,int h_c){
	if(list->root == NULL){
...
node* shift(p_queue *list){
	if(list->root == NULL){
		...
		node *iter = list->root;
		while(iter != NULL){
			if((min->cost + min->h_cost) > (iter->cost + iter->h_cost)){
...
bool a_star(graph *g,p_queue *list,int goal){
	int at,cost;
	node *n;
	...
	for(int to = 0;to < g->v_count;to++){
		if(g->adj_matrix[at][to] && !g->isvisited[to]){
			g->prev_list[to] = at;
			push(list,to,cost + g->adj_matrix[at][to],h(to));
			g->isvisited[to] = 1;
		}
	}

	if(a_star(g,list,goal)){
		return 1;
	}
	return 0;
}
